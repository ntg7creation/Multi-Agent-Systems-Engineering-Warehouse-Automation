...
function AnalyticsView({ analytics, refreshAnalytics, onDownloadAnalytics }) {
    ...
    <main className="analytics-layout">
      <section className="analytics-header">
        ...
        <div className="analytics-actions">
          ...
          <button className="download-button" type="button" onClick={() => onDownloadAnalytics('json')}>Download JSON</button>
          <button className="download-button" type="button" onClick={() => onDownloadAnalytics('csv')}>Download CSV</button>
        </div>
      </section>
...
function App() {
  ...
  async function refreshState({ forceConfig = false } = {}) {
    try {
      ...
      setError(err.message || 'Could not initialize the browser Python simulation.')
    }
  }
  ...
  async function handleAnalyticsDownload(format) {
    try {
      setExportStatus(`Preparing ${format.toUpperCase()}...`)
      await downloadAnalytics(format)
    ...
    <div className="page">
      <header className="topbar">
        <div className="title-block">
          ...
          <p className="subtitle">{state?.scenario?.name ?? 'Browser Python simulation dashboard'}</p>
        </div>
      ...
      {activeTab === 'analytics' ? (
        <AnalyticsView
          ...
          onDownloadAnalytics={handleAnalyticsDownload}
        />
      ) : (
        <>
